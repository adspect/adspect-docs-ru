...
| Макрос          | Описание |
|:----------------|:---------|
| `{sid}`         | Идентификатор потока. |
| `{aid}`         | Идентификатор аккаунта. |
| `{host}`        | Доменное имя запрошенного сайта. |
...
| `{useragent}`   | [Строка user agent](https://ru.wikipedia.org/wiki/User_agent) посетителя. |
| `{referrer}`    | [Referrer](https://developer.mozilla.org/ru/docs/Web/API/Document/referrer) посетителя. |
| `{cost}`        | Цена перехода. |
| `{subid}`       | Идентификатор субаккаунта. |
| `{clickid}`     | Уникальный идентификатор клика (внешний из параметра ссылки, либо сгенерированный Adspect). |
| `{country}`     | [Двухбуквенный код страны](https://ru.wikipedia.org/wiki/ISO_3166-1_alpha-2) посетителя. |
...
| `{tags}`        | [Теги обработки перехода](reporting.md#id3), если есть. |
| `{p:параметр}`  | Значение указанного параметра из URL запроса. |
| `{h:заголовок}` | Значение указанного заголовка запроса. |
...
Возвращает 200 OK с [заголовком X-Accel-Redirect](https://www.nginx.com/resources/wiki/start/topics/examples/x-accel/) ---
это механизм внутрисерверного перенаправления, поддерживаемый веб-серверами NGINX и Cherokee.
...
Данный блок параметров управляет сопоставлением URL-параметров для встроенного в Adspect трекера. Настраивать
