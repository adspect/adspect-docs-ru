...
При создании потока вам нужно выбрать цель использования. Очень важно выбрать ту, которая максимально точно соответствует вашей задаче --- от этого напрямую зависит результат. Настройки по умолчанию уже оптимизированы под выбранную цель. Заполнять все поля необязательно --- обычно достаточно указать [контент и белую страницу](#money-and-safe-pages), а остальное Adspect сделает сам.
...
В этом поле вы можете указать до 32 мнемонических тегов, которые тем или иным образом описывают суть потока, например: "gambling",
"betting", "nutra", "тест" и т.п. Вы можете делать поиск по этим тегам при помощи поля "Поиск" в левом углу списка потоков.
...
* - Фильтр
  - Основной режим работы, в котором происходит фильтрация переходов в реальном времени. Все технологии Adspect, в том числе
    [машинное обучение VLA™](how-it-works.md#machine-learning), работают только в этом режиме.

* - Модерация
  - Специальный режим, который **должен быть включен**, когда ваша рекламная кампания находятся на модерации --- он гарантирует,
    что Adspect никак не повлияет на процесс одобрения кампании. Каждому посетителю будет показана белая страница.<br><br>
    **Не переходите по ссылке потока, когда включен этот режим, иначе вы автоматически попадете в блок.**

* - Контент
  - Вспомогательный режим, в котором всем посетителям показывается контент. Он удобен для тестирования настроек контента.

* - Белая&nbsp;страница
  - Вспомогательный режим, в котором всем посетителям показывается белая страница. Он удобен для тестирования настроек белой
    страницы.  Включайте этот режим, когда рекламная кампания остановлена.
:::
...
Adspect поддерживает множество макросов (шаблонов подстановки), которые можно использовать в разных полях настроек потока,
чтобы автоматически подставлять значения разных атрибутов каждого клика. Ниже --- таблица доступных макросов с примерами:

:::{list-table} Макросы Adspect
:header-rows: 1
...
* - `{asn}`
  - [Номер автономной системы (ASN)](https://ru.wikipedia.org/wiki/%D0%90%D0%B2%D1%82%D0%BE%D0%BD%D0%BE%D0%BC%D0%BD%D0%B0%D1%8F_%D1%81%D0%B8%D1%81%D1%82%D0%B5%D0%BC%D0%B0_(%D0%98%D0%BD%D1%82%D0%B5%D1%80%D0%BD%D0%B5%D1%82)) Интернет-провайдера посетителя.
  - `65536`

* - `{scheme}`
  - Протокол из ссылки клика: `https` или `http`.<br><br>
    *Может отличаться от протокола в адресной строке браузера, если вы используете CDN.*
  - Если ссылка клика `https://example.com/`:<br><br>
    `{scheme}` → `https`
...
* - `{path}`
  ...
  - Если ссылка клика `https://example.com/posts/42`:<br><br>
    `{path}` → `/posts/42`
...
* - `{query}`
  ...
  - Если ссылка клика `https://example.com/?utm_campaign=Dating&utm_medium=banner`:<br><br>
    `{query}` → `utm_campaign=Dating&utm_medium=banner`

* - `{p:параметр}`
  - Значение URL-параметра клика с именем `параметр`.
  - Если ссылка клика `https://example.com/?utm_campaign=Dating&utm_medium=banner`:<br><br>
    `{p:utm_campaign}` → `Dating`<br>
    `{p:utm_medium}` → `banner`

* - `{h:заголовок}`
  - Значение HTTP-заголовка клика с именем `заголовок` (регистр символов в имени не учитывается).
  - `{h:accept-language}` → `en;q=0.7`

* - `{c:cookie}`
  - Значение cookie клика с именем `cookie` (регистр символов в имени не учитывается).
  - Если имеем заголовок `Cookie: username=guest; language=en-US`:<br><br>
    `{c:username}` → `guest`<br>
    `{c:language}` → `en-US`

* - `{useragent}`
  - [Строка user agent](https://ru.wikipedia.org/wiki/User_agent) браузера посетителя.
  - `Mozilla/5.0 (iPhone; CPU iPhone OS 18_6 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/18.6 Mobile/15E148 Safari/604.1`

* - `{referrer}`
  - [URL реферера](https://developer.mozilla.org/ru/docs/Web/HTTP/Reference/Headers/Referer) клика.
  - `https://www.google.com/search?q=cat`

* - `{country}`
  - [Двухбуквенный код страны](https://ru.wikipedia.org/wiki/ISO_3166-1_alpha-2) посетителя.
  - `AE`
...
* - `{clickid}`
  - [Click ID](#click-id) (внешний или сгенерированный).
  - Если [Click ID](#click-id) --- `{p:gclid}` и в URL есть `gclid=EAIaIQobChMI&hellip;BwE`:<br><br>
    `{clickid}` → `EAIaIQobChMI&hellip;BwE`

* - `{cost}`
  - [Цена клика](#click-cost).
  - Если [Click Cost](#click-cost) --- `{p:cost}` и в URL есть `cost=0.015`:<br><br>
    `{cost}` → `0.015`

* - `{subid}`
  - [Sub ID](#sub-id) клика.
  - Если [Sub ID](#sub-id) --- `{p:utm_campaign}` и в URL есть `utm_campaign=Dating`:<br><br>
    `{subid}` → `Dating`

* - `{tags}`
  - Разделенные пробелами [теги клика](reporting.md#tags) (сработавшие фильтры).
  - `BOT GEO REF`
...
Оба поля поддерживают [макросы](#macros), например:
```
...
[URL-кодируются](https://developer.mozilla.org/ru/docs/Glossary/Percent-encoding) перед подстановкой в ссылку, т.к.
...
Контент --- это ваш настоящий сайт, лендинг или CPA-оффер, который вы собираетесь рекламировать и который приносит вам доход.
Вы можете указать до 32 контент-страниц для сплит-тестирования. Трафик будет распределяться между ними в соответствии с правилами
выбранного [ротатора](#money-page-rotator).

В зависимости от выбранного [действия](#action), это поле может содержать разные значения:
...
* **Путь к локальному файлу или директории,** например `page.php` или `/landers/page.html`. Слово "локальный" в данном
  ...
  то есть на том же домене, который будет использоваться для конечной ссылки.
...
Каждая контент-страница имеет свой "вес", который по умолчанию равен 10. Этот параметр используется только при сплит-тестировании
нескольких контент-страниц. Конкретный смысл веса зависит от выбранного [ротатора](#money-page-rotator). Если у вас только одна
контент-страница, то этот параметр игнорируется.
...
:::{important}
Одно и то же действие может работать по-разному при PHP- и JavaScript-интерации. В таких случаях описание работы для
JavaScript-интеграции приводится в отдельном блоке текста после общего описания, которое работает только при PHP-интеграции.
:::
...
* - Относительный путь
  - `page.php`<br>
    `folder/page.html`
  - Путь относительно папки, в которую загружен PHP-файл интеграции Adspect.<br><br>
    Например, если указан путь `landers/page.html` и загрузить PHP-файл Adspect по ссылке `https://example.com/ads/index.php`, Adspect отобразит страницу `https://example.com/ads/landers/page.html`.
...
* - URL
  ...
  - Путь из URL будет использован относительно корня текущего домена. Схема и домен в URL игнорируются.<br><br>
    Например, если указан URL `https://dummy.test/landers/page.html` и загрузить PHP-файл Adspect по ссылке `https://example.com/ads/index.php`, Adspect отобразит страницу `https://example.com/landers/page.html`.
:::
...
* - `/` (папка)
  - `folder/`<br>
    `folder`
  - Adspect отобразит файл `index.php`, `index.html` или `index.htm` внутри папки, в указанном порядке.
...
* - `.php`<br>
    ...
    `.php3`
  - `page.php`
  - Файл будет выполнен как PHP-скрипт при помощи PHP-директивы [`require`](https://www.php.net/manual/ru/function.require.php).
...
При отображении страниц как локальных файлов вы также можете добавить параметры ссылки с [макросами](#macros) после имени файла,
и они будут переданы в PHP-код, где будут доступны через [суперглобальную переменную `$_GET`](https://www.php.net/manual/ru/reserved.variables.get.php). Например:
```
...
:::{important}
При JavaScript-интеграции это действие выполняет [JavaScript-редирект replace()](streams.md#js-replace).
:::
...
[HTTP 303 See Other](https://developer.mozilla.org/ru/docs/Web/HTTP/Status/303) --- вид перенаправления, который работает
по принципу HTTP 302 Found.
...
[HTTP 307 Temporary Redirect](https://developer.mozilla.org/ru/docs/Web/HTTP/Status/307) --- вид перенаправления, который работает
по принципу HTTP 302 Found.
...
[HTTP 308 Permanent Redirect](https://developer.mozilla.org/ru/docs/Web/HTTP/Status/308) --- вид перенаправления, который работает
по принципу HTTP 301 Moved Permanently.
...
JavaScript-редирект при помощи [метода `location.assign()`](https://developer.mozilla.org/ru/docs/Web/API/Location/assign).
...
JavaScript-редирект при помощи [метода `location.replace()`](https://developer.mozilla.org/ru/docs/Web/API/Location/replace).
...
JavaScript-редирект, который открывает ссылку в родительском окне браузера, даже если заклоаченная ссылка была открыта во фрейме или
в iframe, тем самым осуществляя выход из фрейма.
Если фрейма нет, то этот редирект аналогичен [JavaScript-редиректу assign()](streams.md#js-assign).

:::{important}
Этот трюк не сработает в [iframe с атрибутом sandbox](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/iframe#sandbox).
:::
...
Далее этот же отпечаток пройдет вероятностный анализ нашей [моделью машинного обучения VLA™](how-it-works.md#machine-learning).

:::{note}
Данная функциональность не реализована в [нативной интеграции с Keitaro](keitaro.md) из-за технических ограничений системы пользовательских фильтров Keitaro.
:::
...
Эта настройка пропускает только посетителей с устройствами с сенсорным экраном (мобильными телефонами и планшетами).
Если настройка включена, а поддержка сенсорного экрана отсутствует, то посетитель будет заблокирован. Это бывает удобно
...
:::{note}
Эта проверка работает только при включенной [фильтрации по JavaScript-отпечаткам](#js-fingerprinting).
:::
...
Эта настройка пропускает трафик из мобильных приложений в общем порядке, не считая его априори фродовым.
...
Эта настройка пропускает посетителей, использующих технологию [Google IP Protection](https://privacysandbox.google.com/protections/ip-protection) (также известную как Chrome Privacy Sandbox) для сокрытия своих настоящих IP-адресов за специальными прокси-серверами Google. Включите ее, если вы наблюдаете повышенный объем заблокированного трафика, приходящего с IP-адресов Google.

(allow-apple-proxy)=
### Пропускать трафик Apple iCloud Private Relay

Эта настройка пропускает посетителей, использующих услугу [iCloud Private Relay](https://support.apple.com/en-us/102602) от Apple для сокрытия своих настоящих IP-адресов за специальными прокси-серверами Apple. Включите ее, если вы наблюдаете повышенный объем заблокированного трафика, приходящего с устройств Apple iOS.

(countries-and-devices)=
## Страны и устройства

Эти поля позволяют вам ограничить круг потенциальных посетителей контента только указанными странами, операционными системами,
браузерами, движками и языками браузера. Если оставить поле пустым, то проверка по нему не производится (пропускает всех).
...
Данный блок настроек позволяет вам фильтровать трафик по спискам IP-адресов, диапазонов IP-адресов, [CIDR-префиксов](https://ru.wikipedia.org/wiki/%D0%91%D0%B5%D1%81%D0%BA%D0%BB%D0%B0%D1%81%D1%81%D0%BE%D0%B2%D0%B0%D1%8F_%D0%B0%D0%B4%D1%80%D0%B5%D1%81%D0%B0%D1%86%D0%B8%D1%8F) и [номеров автономных систем (ASN)](https://ru.wikipedia.org/wiki/%D0%90%D0%B2%D1%82%D0%BE%D0%BD%D0%BE%D0%BC%D0%BD%D0%B0%D1%8F_%D1%81%D0%B8%D1%81%D1%82%D0%B5%D0%BC%D0%B0_(%D0%98%D0%BD%D1%82%D0%B5%D1%80%D0%BD%D0%B5%D1%82)). У каждого потока есть два списка: белый и черный. Разделяйте отдельные элементы списка переносами строки или пробелами.
...
Черный и белый списки user agent позволяют вам фильтровать посетителей по их [user agent](https://developer.mozilla.org/ru/docs/Web/HTTP/Reference/Headers/User-Agent). В списках указываются [Perl-совместимые регулярные выражения (PCRE)](http://www.pcre.ru/docs/perl/text/intro/) по одному на строку. Они проверяются по очереди до первого совпадения. Сравнение происходит с учетом регистра символов.
...
Черный и белый списки referrer позволяют вам фильтровать посетителей по их [referrer](https://developer.mozilla.org/ru/docs/Web/HTTP/Reference/Headers/Referer). В списках указываются [Perl-совместимые регулярные выражения (PCRE)](http://www.pcre.ru/docs/perl/text/intro/) по одному на строку. Они проверяются по очереди до первого совпадения. Сравнение происходит с учетом регистра символов.
...
URL-правила позволяют проверять, добавлять, изменять или удалять параметры в ссылках у входящих кликов. С их помощью можно,
например, блокировать клик, если у него есть или нет нужного параметра, проверять параметры на совпадение или несовпадение
с регулярным выражением и т.п.

Правила применяются по порядку --- сверху вниз, как они указаны в настройках потока. Каждое следующее правило работает с той
ссылкой, которая получилась в результате применения предыдущих. Например, если одно из правил удаляет параметр из ссылки, то
следующие уже не смогут его проверить или изменить. Если же правило добавляет новый параметр, то следующие смогут с ним работать.

Правила работают с той ссылкой, по которой пришёл клик, а не с конечной ссылкой на контент или белую страницу. Поэтому любые
манипуляции с параметрами (добавление, изменение, удаление) не повлияют на конечную ссылку, если только вы не включите для нее
[проброс параметров](#parameter-passthrough).
...
* - URL-параметр
  - Имя URL-параметра, с которым будет работать правило. Например, если в ссылке есть параметр `utm_medium=banner`, то имя
    параметра --- `utm_medium`.

* - Оператор
  - Конкретная проверка или операция, которая будет произведена с URL-параметром.
    См. [таблицу операторов](#url-rule-operators) ниже.

* - Аргумент
  - Аргумент оператора, если он предусмотрен конкретным оператором (см. [таблицу операторов](#url-rule-operators) ниже).
    В этом поле поддерживаются [макросы](#macros).
...
* - совпадает с regexp
  - Проверяет параметр на совпадение с [Perl-совместимым регулярным выражением (PCRE)](http://www.pcre.ru/docs/perl/text/intro/) в аргументе (с учетом регистра символов).
...
Если эта настройка включена, то Adspect будет блокировать всех посетителей, у которых часовой пояс браузера
