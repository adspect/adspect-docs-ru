...
* - Относительный путь
  ...
  - Путь относительно папки, в которую загружен PHP-файл интеграции Adspect.<br><br>
    Например, если указан путь `landers/landing.html` и загрузить PHP-файл Adspect по ссылке `https://example.com/ads/index.php`, Adspect отобразит страницу `https://example.com/ads/landers/landing.html`.
...
* - URL
  ...
  - Путь из URL будет использован относительно корня текущего домена. Схема и домен в URL игнорируются.<br><br>
    Например, если указан URL `https://dummy.test/landers/landing.html` и загрузить PHP-файл Adspect по ссылке `https://example.com/ads/index.php`, Adspect отобразит страницу `https://example.com/landers/landing.html`.
:::
...
:::{tip}
Это действие наиболее безопасно, и мы настоятельно рекомендуем использовать его везде, где это технически возможно.
:::
