...
Впрочем, этот лимит во многом условный, мы можем поднять или вовсе снять его по вашему запросу. Свяжитесь с нами,
если вам потребуется больше потоков.
...
*Не называйте* файл `index.html`! Не считая того, что это легко угадываемое имя и позволяет легко «пробить клоаку»,
это имя файла может быть выбрано вашим веб-сервером вместо `index.php` в качестве индексного файла, когда в URL
перехода отсутствует явное имя файла после символа слэша: например, `https://example.com/lander/` может быть обработан
как `https://example.com/lander/index.html` вместо `https://example.com/lander/index.php`. То или иное
поведение зависит от настроек веб-сервера, поэтому в общем случае нельзя полагаться на что-то конкретное.
...
Мы рекомендуем включить VLA для всех потоков. 95% является оптимальным начальным значением для точности VLA.
