...
:::{list-table} Список поддерживаемых макросов
:header-rows: 1
...
* - `{referrer}`
  - [Referrer](https://developer.mozilla.org/ru/docs/Web/HTTP/Reference/Headers/Referer).
...
и они будут переданы в PHP-код, где будут доступны через
...
```
Далее значение этого макроса может быть использовано в коде страницы `page.php` следующим образом:
```php
